...
main(int argc, char *argv[]) {
    ...
    pipeline.tiler_sink_pad = gst_element_get_static_pad(pipeline.tiler, "sink");
    if (!pipeline.tiler_sink_pad)
        g_print("Unable to get src pad\n");
    else
        gst_pad_add_probe(pipeline.tiler_sink_pad, GST_PAD_PROBE_TYPE_BUFFER,
                          tiler_sink_pad_buffer_probe, (gpointer) pipeline.fpsdisplay, NULL);
    gst_object_unref(pipeline.tiler_sink_pad);
