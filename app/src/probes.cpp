...
void checkInactiveSource(vector<int> source_ids) {
    ...
    for (int i = 0; i < srcmanager.allSources.size(); i++) {
        ...
        if (srcmanager.allSources[i]->consecutiveMissedFrames > 50) {
...
tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        polygon = source->polygon;  //we get the polygon of that frame
        polygonSides = source->polygon_sides;
        display_meta = nvds_acquire_display_meta_from_pool(
                batch_meta); //we get the display meta.. from  the batch meta?? hmm nasty but its a prototype version i guess
